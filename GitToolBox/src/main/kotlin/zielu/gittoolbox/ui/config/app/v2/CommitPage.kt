...
internal class CommitPage : GtFormUiEx<GitToolBoxConfig2> {
  ...
  override fun fillFromState(state: GitToolBoxConfig2) {
    ...
    commitMessageValidation.set(state.commitMessageValidationEnabled)
    commitMessageValidationRegex.set(state.commitMessageValidationRegex)
    completionFormattersContent = state.completionConfigs.map { it.copy() }
