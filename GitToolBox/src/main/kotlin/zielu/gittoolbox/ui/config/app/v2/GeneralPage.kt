...
import zielu.gittoolbox.ui.config.app.PresenterPreview
...
internal class GeneralPage(
  ...
  override fun fillFromState(state: GitToolBoxConfig2) {
    presentationMode.set(state.getPresenter())
    showStatusWidget.set(state.showStatusWidget)
    showChangesInStatusBar.set(state.showChangesInStatusBar)
    showBlameWidget.set(state.showBlameWidget)
    showEditorInlineBlame.set(state.showEditorInlineBlame)
    showProjectViewDecoration.set(state.showProjectViewStatus)
    behindTracker.set(state.behindTracker)
    updateProjectAction.set(state.getUpdateProjectAction())
    absoluteDateTypeStyle.set(state.absoluteDateTimeStyle)
  }
